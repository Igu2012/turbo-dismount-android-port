...
public class VungleManager : MonoBehaviour
{
	...
	public static void onEvent(string e, string arg)
	{
		...
		if (e == "OnAdEnd")
		{
			...
			OnAdFinishedEvent(array[1], adWinFinishedEventArgs);
		}
		if (e == "OnAdPlayableChanged")
		{
			string[] array2 = arg.Split(':');
			OnAdPlayableEvent(array2[1], "1".Equals(array2[0]));
		}
		if (e == "Diagnostic")
		{
			OnSDKLogEvent(arg);
	...
	private void OnPlacementPrepared(string param)
	{
		...
		OnPlacementPreparedEvent(arg, arg2);
	}
	...
	private void OnVungleCreative(string param)
	{
		...
		OnVungleCreativeEvent(arg, arg2);
	}
	...
	{
		AdFinishedEventArgs e = new AdFinishedEventArgs();
		Dictionary<string, object> dictionary = (Dictionary<string, object>)Json.Deserialize(param);
		e.WasCallToActionClicked = extractBoolValue(dictionary, "wasCallToActionClicked");
		e.IsCompletedView = extractBoolValue(dictionary, "wasSuccessFulView");
		e.TimeWatched = 0.0;
		OnAdFinishedEvent(dictionary["placementID"].ToString(), e);
	}
