...
public static class IAP
{
	...
	static IAP()
	{
		...
		GoogleIABManager.queryInventorySucceededEvent += (List<GooglePurchase> purchases, List<GoogleSkuInfo> skus) =>
		{
		...
		GoogleIABManager.queryInventoryFailedEvent += (string error) =>
		{
		...
		GoogleIABManager.purchaseSucceededEvent += (GooglePurchase purchase) =>
		{
		...
		GoogleIABManager.purchaseFailedEvent += (string error, int response) =>
		{
		...
		GoogleIABManager.consumePurchaseSucceededEvent += (GooglePurchase purchase) =>
		{
			if (_purchaseCompletionAction != null)
			{
				_purchaseCompletionAction(true, null);
		...
		GoogleIABManager.consumePurchaseFailedEvent += (string error) =>
		{
			if (_purchaseCompletionAction != null)
			{
