...
namespace Rewired.Demos
{
	...
	public class ControlRemappingDemo1 : MonoBehaviour
	{
		...
		private void DrawMouseAssignment()
		{
			...
			bool flag3 = GUILayout.Toggle(flag2, "Assign Mouse", "Button", GUILayout.ExpandWidth(false));
			if (flag3 != flag2)
			{
				if (flag3)
				{
		...
		private void DrawJoystickSelector()
		{
			...
			bool flag3 = GUILayout.Toggle(flag2, "None", "Button", GUILayout.ExpandWidth(false));
			if (flag3 != flag2)
			{
			...
			if (selectedPlayer != null)
			{
				foreach (Joystick joystick in ReInput.controllers.Joysticks)
				{
					...
					flag3 = GUILayout.Toggle(flag2, joystick.name, "Button", GUILayout.ExpandWidth(false));
					if (flag3 != flag2)
					{
		...
		private void DrawControllerSelector()
		{
			...
			bool flag3 = GUILayout.Toggle(flag2, "Keyboard", "Button", GUILayout.ExpandWidth(false));
			if (flag3 != flag2)
			{
				selectedController.Set(0, ControllerType.Keyboard);
				ControllerSelectionChanged();
			...
			flag3 = GUILayout.Toggle(flag2, "Mouse", "Button", GUILayout.ExpandWidth(false));
			if (flag3 != flag2)
			{
		...
		private void DrawMapCategories()
		{
			...
			foreach (InputMapCategory userAssignableMapCategory in ReInput.mapping.UserAssignableMapCategories)
			{
				...
				bool flag3 = GUILayout.Toggle(flag2, (!(userAssignableMapCategory.descriptiveName != string.Empty)) ? userAssignableMapCategory.name : userAssignableMapCategory.descriptiveName, "Button", GUILayout.ExpandWidth(false));
				if (flag3 != flag2)
				{
					selectedMapCategoryId = userAssignableMapCategory.id;
					selectedMap = selectedPlayer.controllers.maps.GetFirstMapInCategory(selectedController.type, selectedController.id, userAssignableMapCategory.id);
		...
		private void DrawCalibrationWindow(string title, string message)
		{
			...
			for (int i = 0; i < axisElementIdentifiers.Count; i++)
			{
				...
				bool flag3 = GUILayout.Toggle(flag2, controllerElementIdentifier.name, "Button", GUILayout.ExpandWidth(false));
				if (flag2 != flag3)
				{
			...
			if (calibration.selectedElementIdentifierId >= 0)
			{
				...
				GUILayout.Space(15f);
				bool flag4 = GUILayout.Toggle(axis.enabled, "Enabled", "Button", GUILayout.ExpandWidth(false));
				if (axis.enabled != flag4)
				{
					axis.enabled = flag4;
				}
				GUILayout.Space(10f);
				bool flag5 = GUILayout.Toggle(calibration.recording, "Record Min/Max", "Button", GUILayout.ExpandWidth(false));
				...
				bool flag6 = GUILayout.Toggle(axis.invert, "Invert", "Button", GUILayout.ExpandWidth(false));
