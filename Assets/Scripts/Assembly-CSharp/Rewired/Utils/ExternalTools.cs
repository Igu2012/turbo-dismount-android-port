...
namespace Rewired.Utils
{
	...
	{
		private const int SDK_VERSION_HONEYCOMB = 9;

		private const int SDK_VERSION_KITKAT = 19;
		...
		public void GetDeviceVIDPIDs(out List<int> vids, out List<int> pids)
		{
			...
			try
			{
				...
				AndroidJavaClass androidJavaClass2 = new AndroidJavaClass("android.view.InputDevice");
				int[] array = null;
				using (AndroidJavaObject androidJavaObject = androidJavaClass2.CallStatic<AndroidJavaObject>("getDeviceIds", new object[0]))
				{
				...
				for (int i = 0; i < array.Length; i++)
				{
					try
					{
						using (AndroidJavaObject androidJavaObject2 = androidJavaClass2.CallStatic<AndroidJavaObject>("getDevice", new object[1] { array[i] }))
						{
		...
		public object InitializePlatform(ConfigVars configVars)
		{
			return null;
		}
	}
