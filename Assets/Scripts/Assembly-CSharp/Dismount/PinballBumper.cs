	...
	public class PinballBumper : MonoBehaviour
	{
		...
		private void OnCollisionEnter(Collision collision)
		{
			...
			if (!(attachedRigidbody == null) && !(attachedRigidbody.mass < 1.1f) && !(collision.relativeVelocity.sqrMagnitude < 4f))
			{
				...
				Vector3 direction = point - originalPosition;
				direction = base.transform.InverseTransformDirection(direction);
				direction = new Vector3(direction.x, direction.y * 0.2f, direction.z);
				direction = base.transform.TransformDirection(direction).normalized;
				targetPosition = originalPosition + direction * targetMove;
